...
class AppState extends ChangeNotifier {
  ...
  PhoneListing? _listingFromRow(Map<String, dynamic> row,
    ...
    try {
      ...
      return PhoneListing(
        ...
        imageUrls: _listingImagesFromRow(row),
        seller: SellerInfo(
  ...
  List<String> _listingImagesFromRow(Map<String, dynamic> row) {
    for (final key in const ['image_urls', 'imageUrls', 'image_paths', 'imagePaths']) {
      final urls = _imageUrlsFromValue(row[key]);
  ...
  List<String> _imageUrlsFromValue(dynamic value) {
    if (value is Iterable) {
      return value
          .expand<String>((item) {
            if (item is Map) {
              final nested = item['url'] ?? item['publicUrl'] ?? item['public_url'] ?? item['path'] ?? item['name'];
    ...
    if (value is String && value.trim().isNotEmpty) {
      ...
      try {
        ...
        // دعم القيم القديمة بصيغة PostgreSQL array مثل {path1,path2}.
        if (raw.startsWith('{') && raw.endsWith('}')) {
          final inner = raw.substring(1, raw.length - 1);
          return _imageUrlsFromValue(inner.split(','));
        }
...
''), '');
    ...
    final path = normalized.startsWith('listing-images/')
        ? normalized.substring('listing-images/'.length)
        : normalized;
    return Supabase.instance.client.storage.from('listing-images').getPublicUrl(path);
  }
