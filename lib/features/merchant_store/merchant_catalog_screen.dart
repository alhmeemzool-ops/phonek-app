...
/// إدارة كتالوج التاجر: إضافة، تعديل، تجميد/إلغاء تجميد، وحذف.
/// الإعلان المجمد يبقى تحت إدارة التاجر لكنه لا يظهر للزوار.
...
class _MerchantCatalogScreenState extends State<MerchantCatalogScreen> {
  ...
  Future<void> _load() async {
    ...
    if (userId == null) {
      if (mounted) setState(() { _loading = false; _error = 'يجب تسجيل الدخول لإدارة الكتالوج'; });
      return;
    }
    if (mounted) setState(() { _loading = true; _error = null; });
    try {
      final rows = await Supabase.instance.client.from('listings').select('*, profiles!listings_seller_id_fkey(*)').eq('seller_id', userId).order('created_at', ascending: false);
      _rows = (rows as List).whereType<Map<String, dynamic>>().toList();
    } on PostgrestException catch (e) { _error = e.message; }
    catch (e) { _error = '$e'; }
    finally { if (mounted) setState(() => _loading = false); }
  }

  Future<void> _add() async {
    await Navigator.push(context, MaterialPageRoute(builder: (_) => const AddPhoneScreen()));
  ...
  Future<void> _setFrozen(Map<String, dynamic> row, bool frozen) async {
    ...
    try {
      await Supabase.instance.client.from('listings').update({'status': frozen ? 'frozen' : 'active'}).eq('id', row['id']).eq('seller_id', userId);
      await _load();
      if (mounted) _message(frozen ? 'تم تجميد الإعلان' : 'تم إلغاء تجميد الإعلان');
    } catch (e) { if (mounted) _message('تعذر تغيير حالة الإعلان: $e'); }
  }

  Future<void> _delete(Map<String, dynamic> row) async {
    final ok = await showDialog<bool>(context: context, builder: (dialogContext) => AlertDialog(
      title: const Text('حذف الإعلان؟'),
      content: const Text('سيختفي الإعلان من الكتالوج والسوق.'),
    ...
    } catch (e) { if (mounted) _message('تعذر حذف الإعلان: $e'); }
  }
  ...
  Future<void> _edit(Map<String, dynamic> row) async {
    ...
    if (userId == null) return;
    try {
      await Supabase.instance.client.from('listings').update({'title': title.text.trim(), 'price': int.tryParse(price.text.replaceAll(',', '')) ?? row['price'], 'city': city.text.trim(), 'description': description.text.trim()}).eq('id', row['id']).eq('seller_id', userId);
      await _load();
      if (mounted) _message('تم تحديث الإعلان');
    } catch (e) { if (mounted) _message('تعذر تعديل الإعلان: $e'); }
  }
  ...
  PhoneListing? _listingFromRow(Map<String, dynamic> row) {
    ...
    try {
      return PhoneListing(id: row['id'] as String, title: row['title'] as String? ?? '', brand: row['brand'] as String? ?? '', price: (row['price'] as num?)?.toInt() ?? 0, priceIsNegotiable: row['price_is_negotiable'] as bool? ?? true, priceOnCall: row['price_on_call'] as bool? ?? false, oldPrice: (row['old_price'] as num?)?.toInt(), storage: row['storage'] as String? ?? '', ram: row['ram'] as String? ?? '', batteryHealthPercent: (row['battery_health_percent'] as num?)?.toInt(), condition: _condition(row['condition'] as String?), damageNotes: row['damage_notes'] as String?, hasBox: row['has_box'] as bool? ?? false, hasCharger: row['has_charger'] as bool? ?? false, hasInvoice: row['has_invoice'] as bool? ?? false, hasEarphones: row['has_earphones'] as bool? ?? false, warranty: WarrantyType.none, city: row['city'] as String? ?? '', imageUrls: (row['image_urls'] as List?)?.whereType<String>().toList() ?? const [], seller: SellerInfo(id: row['seller_id'] as String? ?? '', name: profile['name'] as String? ?? 'متجر PhoneK', phone: profile['phone'] as String? ?? '', city: profile['city'] as String? ?? row['city'] as String? ?? '', isShop: profile['is_shop'] as bool? ?? true, isVerifiedStore: profile['is_verified_store'] as bool? ?? false), status: _status(row['status'] as String?), createdAt: DateTime.tryParse('${row['created_at']}') ?? DateTime.now(), viewCount: (row['view_count'] as num?)?.toInt() ?? 0, isFeatured: row['is_featured'] as bool? ?? false, description: row['description'] as String? ?? '');
    } catch (_) { return null; }
  }

  DeviceCondition _condition(String? value) { switch (value) { case 'new': case 'newDevice': return DeviceCondition.newDevice; case 'minor_scratches': case 'minorScratches': return DeviceCondition.minorScratches; case 'cracked': return DeviceCondition.cracked; default: return DeviceCondition.excellent; } }
  ListingStatus _status(String? value) { switch (value) { case 'sold': return ListingStatus.sold; case 'frozen': return ListingStatus.frozen; case 'expired': return ListingStatus.expired; case 'pending_review': case 'pendingReview': return ListingStatus.pendingReview; default: return ListingStatus.active; } }
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    floatingActionButton: FloatingActionButton.extended(onPressed: _add, icon: const Icon(Icons.add), label: const Text('إضافة إعلان')),
    body: _loading ? const Center(child: CircularProgressIndicator()) : _error != null ? Center(child: Padding(padding: const EdgeInsets.all(24), child: Text('تعذر تحميل الكتالوج\n$_error', textAlign: TextAlign.center))) : _rows.isEmpty ? _empty() : RefreshIndicator(onRefresh: _load, child: ListView.separated(padding: const EdgeInsets.fromLTRB(14, 14, 14, 90), itemCount: _rows.length, separatorBuilder: (_, __) => const SizedBox(height: 12), itemBuilder: (_, index) => _item(_rows[index]))),
  );
