    ...
    return Semantics(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(compact ? 8 : 9999),
    ...
    return GridView.builder(
      ...
      itemBuilder: (_, index) {
        ...
        return InkWell(
          ...
          onTap: () => showMerchantBadgeDetails(context, badge, current: current),
          child: Card(
            elevation: 0,
            ...
            child: Padding(
              ...
              child: Column(children: [
                ...
                Text('المستوى ${badge.level}', style: TextStyle(fontWeight: FontWeight.w800, color: current ? const Color(0xFFFFD700) : null)),
...
void showMerchantBadgeDetails(BuildContext context, MerchantBadge badge, {required bool current}) {
  showModalBottomSheet<void>(
    ...
    builder: (_) => SafeArea(child: Padding(padding: const EdgeInsets.fromLTRB(20, 16, 20, 24), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [Expanded(child: Text(badge.nameAr, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w800))), IconButton(onPressed: () => Navigator.pop(context), icon: const Icon(Icons.close))]),
      Text('المستوى ${badge.level}', style: const TextStyle(color: Color(0xFFFFD700), fontWeight: FontWeight.bold)),
      const SizedBox(height: 12),
      Center(child: MerchantBadgeArt(level: badge.level, size: 100, locked: !current && badge.level > 1)),
      const SizedBox(height: 12),
      Text(badge.descriptionAr, style: const TextStyle(color: Colors.white70, height: 1.5)),
      const SizedBox(height: 14),
      Container(width: double.infinity, padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: const Color(0xFF2A2A2A), borderRadius: BorderRadius.circular(12), border: Border.all(color: current ? const Color(0xFFFFD700) : Colors.white12)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(current ? 'المستوى الحالي' : 'شروط الحصول على الشارة', style: const TextStyle(color: Colors.white60, fontSize: 12)),
        const SizedBox(height: 6),
        Text(badge.requiredSales == 0 ? 'تفعيل المتجر والتحقق من الهوية' : '${badge.requiredSales} عملية بيع ناجحة${badge.requiresLicense ? ' + توثيق رخصة المتجر' : ''}${badge.minRating != null ? ' + تقييم أعلى من ${badge.minRating!.toStringAsFixed(1)}' : ''}', style: const TextStyle(fontWeight: FontWeight.bold)),
      ])),
      const SizedBox(height: 16),
      SizedBox(width: double.infinity, child: FilledButton(onPressed: () => Navigator.pop(context), child: const Text('إغلاق التفاصيل'))),
    ]))),
  );
}
