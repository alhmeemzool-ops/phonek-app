...
class MyListingsScreen extends StatelessWidget {
      ...
      body: listings.isEmpty
          ...
          : ListView.separated(
              ...
              itemBuilder: (context, index) =>
                  _listingTile(context, listings[index]),
            ),
    );
  }
        ...
        child: Row(
          children: [
            ...
            PopupMenuButton<_ListingAction>(
              ...
              itemBuilder: (_) => [
                const PopupMenuItem(
                    value: _ListingAction.editPrice,
                    child: Text('تحديث السعر')),
                const PopupMenuItem(
                    value: _ListingAction.markSold,
                    child: Text('حذف بعد البيع')),
                const PopupMenuItem(
                    value: _ListingAction.open, child: Text('عرض الإعلان')),
              ],
            ),
          ],
        ),
  ...
  Future<void> _handleAction(
      BuildContext context, PhoneListing listing, _ListingAction action) async {
    switch (action) {
      case _ListingAction.open:
        Navigator.push(
            context,
            MaterialPageRoute(
                builder: (_) => PhoneDetailsScreen(listing: listing)));
        return;
  ...
  Future<void> _showEditPriceDialog(
      BuildContext context, PhoneListing listing) async {
    ...
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('تعذر تحديث السعر حالياً')));
      }
    }
  }
  ...
  Future<void> _confirmDelete(
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
            'سيتم حذف إعلان «${listing.title}» نهائياً بعد تأكيد البيع. هل تريد المتابعة؟'),
        actions: [
          TextButton(
    ...
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('تم حذف الإعلان نهائياً')));
      }
    } catch (_) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('تعذر حذف الإعلان حالياً')));
      }
    }
  }
}
  ...
  Widget build(BuildContext context) {
    return const Center(
      child: Padding(
        ...
        child: Text('لا توجد إعلانات في حسابك حالياً',
