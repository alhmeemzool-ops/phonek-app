...
class PhoneDetailsScreen extends StatelessWidget {
            ...
            child: Column(
              ...
              children: [
                ...
                Wrap(
                  ...
                  runSpacing: 7,
                  children: [
                    _chip('الكرتونة', listing.hasBox),
                    _chip('الشاحن', listing.hasCharger),
                    _chip('الفاتورة', listing.hasInvoice),
                    _chip('السماعة', listing.hasEarphones),
                  ],
                ),
                if (listing.damageNotes?.trim().isNotEmpty == true) ...[
                  const SizedBox(height: 18),
                  _section('ملاحظات الأعطال'),
                  Text(listing.damageNotes!, style: const TextStyle(color: AppColors.danger, height: 1.5)),
                ],
                const SizedBox(height: 18),
                _section('الوصف'),
                ...
                if (similar.isNotEmpty) ...[
                  ...
                  SizedBox(
                    ...
                    child: ListView.separated(
                      ...
                      itemBuilder: (_, i) {
                        final phone = similar[i];
                        return SizedBox(
                          width: 150,
                          child: PhoneCard(
                            listing: phone,
                            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => PhoneDetailsScreen(listing: phone))),
                          ),
                        );
                      },
                    ),
  ...
  Widget _heroImage(BuildContext context) {
    return GestureDetector(
      onTap: listing.imageUrls.isEmpty
          ? null
          : () => Navigator.push(context, MaterialPageRoute(builder: (_) => _PhotoGalleryScreen(images: listing.imageUrls, initialIndex: 0, title: listing.title))),
      child: SizedBox(
        height: 300,
        child: Stack(
          fit: StackFit.expand,
          children: [
            listing.imageUrls.isEmpty
                ...
                : CachedNetworkImage(
                    ...
                    fit: BoxFit.cover,
                    errorWidget: (_, __, ___) => const ColoredBox(color: AppColors.surfaceLight, child: Icon(Icons.broken_image)),
                  ),
            if (listing.imageUrls.length > 1)
              Positioned(
                ...
                child: Container(
                  ...
                  child: Text('${listing.imageUrls.length} صور • اضغط للتكبير', style: const TextStyle(color: Colors.white)),
                ),
              ),
          ],
        ),
        ...
        itemBuilder: (_, i) {
          return GestureDetector(
            ...
            child: ClipRRect(
              ...
              child: CachedNetworkImage(imageUrl: listing.imageUrls[i], width: 82, height: 58, fit: BoxFit.cover),
            ),
          );
        },
      ),
  ...
  Widget _specs() {
    final rows = <MapEntry<String, String>>[
      ...
      MapEntry('الضمان', _warranty(listing.warranty)),
    ];
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(12),
        child: Column(children: rows.map((row) => _row(row.key, row.value)).toList()),
      ),
    );
  }
  ...
  Widget _row(String label, String value) => Padding(
        ...
        child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(label, style: const TextStyle(color: AppColors.textSecondary)), Flexible(child: Text(value, textAlign: TextAlign.end, style: const TextStyle(fontWeight: FontWeight.w600))) ]),
      );

  String _warranty(WarrantyType w) => switch (w) {
  ...
  Widget _seller(BuildContext context) {
    ...
    return InkWell(
      ...
      onTap: seller.isShop && seller.id.isNotEmpty
          ? () => Navigator.push(context, MaterialPageRoute(builder: (_) => ShopProfileScreen(shopId: seller.id, initialSeller: seller)))
          : null,
      child: Card(
        child: Padding(
          padding: const EdgeInsets.all(12),
          child: Row(
            children: [
              CircleAvatar(
                ...
                child: seller.avatarUrl?.isNotEmpty == true
                    ? ClipOval(child: Image.network(seller.avatarUrl!, width: 50, height: 50, fit: BoxFit.cover))
                    : Text(AppFormatters.firstChar(seller.name), style: const TextStyle(color: AppColors.gold)),
              ),
                    ...
                    Row(
                      children: [
                        ...
                        if (seller.isVerifiedStore) ...[
                          ...
                          const SizedBox(width: 5),
                          MerchantBadgeChip(level: levelForSales(seller.completedSales), compact: true),
                        ],
                      ],
                    ),
                    const SizedBox(height: 3),
                    Text(seller.isShop ? 'فتح صفحة المتجر' : seller.replySpeedLabel, style: const TextStyle(color: AppColors.textSecondary, fontSize: 12)),
                  ],
                ),
              ),
        ...
        child: Row(
          children: [
            ...
            if (!listing.priceOnCall) ...[
              ...
              Expanded(child: OutlinedButton(onPressed: () => _offer(context), child: const Text('تقديم عرض'))),
            ],
          ],
        ),
      ),
  ...
  Widget _action(IconData icon, String label, VoidCallback onTap) => Padding(
        ...
        child: InkWell(onTap: onTap, child: Padding(padding: const EdgeInsets.all(5), child: Column(mainAxisSize: MainAxisSize.min, children: [Icon(icon, color: AppColors.gold), Text(label, style: const TextStyle(fontSize: 10))]))),
      );
  ...
  Future<void> _whatsapp(BuildContext context) async {
    ...
    if (number.isEmpty) {
      if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('رقم واتساب غير متوفر')));
      return;
    }
  ...
  void _offer(BuildContext context) {
    ...
    showDialog<void>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: TextField(controller: controller, keyboardType: TextInputType.number, decoration: const InputDecoration(hintText: 'السعر المقترح')),
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              try {
                ...
                if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم إرسال العرض')));
              } catch (e) {
                if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('تعذر إرسال العرض: $e')));
              } finally {
                controller.dispose();
              }
            },
        ...
        child: Wrap(
          children: [
            ListTile(
              ...
              title: const Text('نسخ رقم الإعلان'),
              onTap: () async {
